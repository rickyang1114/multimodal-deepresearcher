...
const HTMLRenderer = ({
      ...
      try {
        ...
        const messageHandler = (event: MessageEvent) => {
          ...
          if (event.data.type === 'chartSize') {
            ...
            if (iframe && contentHeight > 0 && contentWidth > 0) {
              ...
              // 确保最小高度
              const minHeightValue = typeof minHeight === 'number' ? minHeight : parseInt(String(minHeight), 10)
        ...
        const timeout = setTimeout(() => {
          if (!isLoaded) {
            ...
            // 设置一个合理的默认高度
            if (iframe) {
              iframe.style.height = typeof minHeight === 'number' ? `${minHeight}px` : `${minHeight}px`
            }
          }
        }, 3000)
  ...
  if (!isMounted) {
    return <div className={`d3-chart-placeholder ${className}`} style={{ height, minHeight, marginTop: '1.5rem', marginBottom: '1.5rem' }}></div>
  }

  if (error) {
    return (
      <div className={`d3-chart-error ${className}`} style={{ padding: '1rem', color: 'red', marginTop: '1.5rem', marginBottom: '1.5rem' }}>
